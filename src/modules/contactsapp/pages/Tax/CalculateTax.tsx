...
import { TaxViewModel,TaxTableViewModel,taxDefaultValue } from '@models/Tax';

const initialOrganizations: SelectOptions[] = [];
const provinceSelection: SelectOptions[] = [{ value: '1', text: 'Alberta' }, { value: '2', text: 'British Columbia' }];
...
export const TaxCalculation: React.FC = () => {
    ...
    const loadTaxData = () => {
        TaxServices.getByProvince((String('Alberta')), Number(135000))
            .then((response) =>
                response.json() as Promise<TaxViewModel>,
            )
            .then((data) => {
                setTaxData(data);
                console.log(data)
